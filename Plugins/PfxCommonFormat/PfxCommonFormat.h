...
struct PfxRigWorldInfo {
	...
	PfxUInt32 maxPrimitives;		// 形状の最大数
...
public:
	...
	const PfxRigBodyInfo       &getRigBodyById(int i) const;
	const PfxRigStateInfo      &getRigStateById(int i) const;
	const PfxRigJointInfo      &getRigJointById(int i) const;
	const PfxRigConvexMeshInfo &getConvexMeshById(int i) const;
	const PfxRigLargeMeshInfo  &getLargeMeshById(int i) const;
	const PfxRigNonContactPairInfo  &getRigNonContactPairById(int i) const;
	...
	PfxMatrix3 calcTotalInertia(PfxRigBodyInfo &body,float scale=1.0f);
};
