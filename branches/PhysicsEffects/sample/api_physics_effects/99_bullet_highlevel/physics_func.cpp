...
void broadphase()
{
	...
	//E Find overlapped pairs
	{
		...
		if(ret != PFX_OK) PFX_PRINTF("pfxFindPairs failed %d\n",ret);
...
void collision()
{
	...
	//E Detect collisions
	{
		...
		if(ret != PFX_OK) PFX_PRINTF("pfxDetectCollision failed %d\n",ret);
	}
}
...
void constraintSolver()
{
	...
	pc.countBegin("setup solver bodies");
	{
		...
		if(ret != PFX_OK) PFX_PRINTF("pfxSetupSolverBodies failed %d\n",ret);
	}
	pc.countEnd();
	...
	pc.countBegin("setup contact constraints");
	{
		...
		if(ret != PFX_OK) PFX_PRINTF("pfxSetupJointConstraints failed %d\n",ret);
	}
	pc.countEnd();
	...
	pc.countBegin("solve constraints");
	{
		...
		if(ret != PFX_OK) PFX_PRINTF("pfxSolveConstraints failed %d\n",ret);
...
void physics_simulate()
{
	...
	if(frame%100 == 0) {
		...
		PFX_PRINTF("frame %3d broadphase %.2f collision %.2f solver %.2f integrate %.2f | total %.2f\n",frame,
...
void createBrick(int id,const PfxVector3 &pos,const PfxQuat &rot,const PfxVector3 &boxSize,PfxFloat mass)
{
	...
	states[id].setMotionType(PfxMotionTypeActive);
...
{
	PfxFloat radius = 1.3f * rotSize * boxSize[0] / PFX_PI;
	...
	for(int i=0;i<stackSize;i++) {
		for(int j=0;j<rotSize;j++) {
			...
			rotY *= PfxQuat::rotationY(PFX_PI/(rotSize*0.5f));
		}

		posY += boxSize[1] * 2.0f;
		rotY *= PfxQuat::rotationY(PFX_PI/(PfxFloat)rotSize);
	}
}
...
void createScenePrimitives()
{
	...
	// convex mesh
	{
		...
		if(ret != PFX_OK) {
			PFX_PRINTF("Can't create gConvex mesh.\n");
		}
...
void createSceneJoints()
{
	...
	for(int i=startId;i<startId+n;i++) {
		...
		PFX_ASSERT(numJoints<NUM_JOINTS);
		numJoints++;
	}
...
void createSceneBoxGround()
{
	...
	states[id].setMotionType(PfxMotionTypeFixed);
	states[id].setRigidBodyId(id);
}
...
void createSceneLandscape()
{
	...
	if(ret != PFX_OK) {
		PFX_PRINTF("Can't create large mesh.\n");
	}
